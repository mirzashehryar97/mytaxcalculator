...
interface CalculatorBadgeHeroProps {
  eyebrow: string;
  ...
  /** Icon repeated on every badge. */
  badgeIcon: LucideIcon;
}

/**
 * Calculator hero with an eyebrow pill and a row of trust badges. The first
 * badge is emphasised (solid white) and the rest are translucent.
 */
export default function CalculatorBadgeHero({
  eyebrow,
  ...
  return (
    <header className="mx-auto mb-8 max-w-4xl animate-fade-up text-center sm:mb-10">
      <span className="inline-flex max-w-full items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 font-medium text-emerald-50 text-xs backdrop-blur-sm sm:text-sm">
        <EyebrowIcon className="h-4 w-4 shrink-0 text-emerald-300" aria-hidden="true" />
        {eyebrow}
      </span>
