...
import { buildBusinessYearComparison } from '@/features/business-tax/lib/presentation';
import type { BusinessTaxpayerType } from '@/features/business-tax/types';

interface BusinessYearComparisonProps {
  netIncome: number;
  taxpayerType: BusinessTaxpayerType;
}

export default function BusinessYearComparison({
  netIncome,
  taxpayerType,
}: BusinessYearComparisonProps) {
  const rows = buildBusinessYearComparison(netIncome, taxpayerType);
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-bold text-gray-900 text-lg">
          {BUSINESS_RESULT_COPY.yearComparisonTitle}
        </h3>
      </div>
      <p className="mt-1.5 text-gray-500 text-sm leading-relaxed">
        {BUSINESS_RESULT_COPY.yearComparisonHelp}
      </p>
