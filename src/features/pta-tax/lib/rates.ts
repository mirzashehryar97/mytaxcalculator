import type { PtaAmountBand, PtaFiscalYear, PtaYearRates } from '@/features/pta-tax/types';
...
/**
 * Rupees per US dollar the page opens with, and the day it was taken. The site
 * is stateless and calls no exchange-rate API, so this is a starting figure the
...
export const PTA_EXCHANGE_RATE_DATE = '4 August 2026';
 ...
 * 100 and serial 3 starting "above 101", so US$ 100.50 falls in no band at all.
 * The same defect repeats at 200/201, 350/351, 500/501 and 700/701, and no
 * Finance Act since has corrected it. `findAmountBand` fills a gap downward,
 * which is the reading that does not charge tax the statute never imposed.
 */
 ...
 * the stale code changes nothing. Serial 1 excludes smartphones and serial 2
 * sweeps them in up to US$ 100, which is why the two lowest bands share a rate
 * for every phone this calculator prices.
 */
const INCOME_TAX_148_2026: readonly PtaAmountBand[] = [
...
const INCOME_TAX_148_2025: readonly PtaAmountBand[] = INCOME_TAX_148_2026.map((band) =>
