...
import { FREELANCER_FAQS, FREELANCER_SECTION_COPY } from '@/features/freelancer-tax/lib/content';

export default function FreelancerFaq() {
  return (
    <section className="mt-16 sm:mt-20" aria-labelledby="freelancer-faq-heading">
      <div className="mb-8 text-center sm:mb-10">
        <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 font-medium text-emerald-100 text-sm backdrop-blur-sm">
          ...
          {FREELANCER_SECTION_COPY.faqEyebrow}
        </span>
        <h2
          id="freelancer-faq-heading"
          className="mt-5 font-bold text-3xl text-white tracking-tight sm:text-4xl"
        >
          {FREELANCER_SECTION_COPY.faqTitle}
        </h2>
        <p className="mx-auto mt-3 max-w-2xl text-emerald-50/80">
          {FREELANCER_SECTION_COPY.faqDescription}
        </p>
      </div>

      <div className="mx-auto max-w-4xl space-y-3">
        {FREELANCER_FAQS.map((faq) => (
          <details
            key={faq.id}
      ...
      <p className="mx-auto mt-8 max-w-3xl text-center text-emerald-50/65 text-xs leading-relaxed">
        {FREELANCER_SECTION_COPY.estimateDisclaimer}
      </p>
    </section>
  );
}
